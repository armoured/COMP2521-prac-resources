...
static int v2notInVS(Vertex *vs, int length, int v2);
static void withinRecursive(Graph g, Vertex s, int d, Vertex *vs, int *num, int orig); 
...
static void withinRecursive(Graph g, Vertex s, int d, Vertex *vs, int *num, int orig) {
        if (d <= 0) return;
        if (*num >= g->nV-1) return;
        int v1 = s;
        int v2;
        for (v2 = 0; v2 < g->nV; v2++) {


            if (g->edges[v1][v2] && v1 != v2) {
                //check vs not in vs already
                if (!v2notInVS(vs, *num, v2) && v2 != orig) {
                    vs[*num] = v2;
                    *num = *num + 1;
                }
            }
        ...
        //if (*num >= g->nV-1) return;
        //foreach element in vs call withinRecursive with d - 1
        int i = 0;
        int length = *num;
        while (i < length) {
            //int length = *num;
            withinRecursive(g, vs[i], d-1, vs, num, orig);
            i++;
        }
...
int within(Graph g, Vertex s, int d, Vertex *vs)
{
   ...
   int num = 0;
   if (d > g->nV) d = g->nV;
   withinRecursive(g, s, d, vs, &num, s);

   //remove starting vertex s from array and do num--
